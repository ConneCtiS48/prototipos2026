...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(false)
    if (err) {
      setError(err.message || 'No se pudo enviar. Revisa el correo o intenta más tarde.')
    } else {
      setSuccess(true)
    }
  }
      ...
      <div className="max-w-md w-full bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 sm:p-8">
        ...
        {success && (
          <div className="mb-4 p-3 bg-green-100 border border-green-400 text-green-700 rounded dark:bg-green-900 dark:text-green-200 text-sm">
            Revisa tu correo; si existe una cuenta con ese email, recibirás un enlace para restablecer la contraseña.
          </div>
        )}
